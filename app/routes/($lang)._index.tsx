...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    { name: "description", content: "Capture and organize your digital world." },
  ];
}
      ...
      <header className="sticky top-0 z-50 w-full border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-sm">
        <div className="w-full px-6 h-16 flex justify-between items-center">
             <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-[#ccff00] flex items-center justify-center text-black font-anton text-xs">S</div>
                <span className="font-anton text-xl uppercase tracking-tighter">{logo}</span>
                <span className="text-[10px] text-zinc-500 ml-2 border border-zinc-800 px-1 rounded-sm">V1.0.0</span>
             </div>

             <div className="flex items-center gap-4">
                 <SignedOut>
                    <Button variant="ghost" className="text-zinc-400 hover:text-[#ccff00] hover:bg-transparent uppercase text-xs tracking-widest" render={
                        <SignInButton><span>[{signIn}]</span></SignInButton>
                     }/>
                     <Button className="bg-zinc-100 text-black hover:bg-[#ccff00] hover:text-black rounded-none h-8 px-4 text-xs uppercase font-bold tracking-widest border border-transparent hover:border-black" render={
                        <SignUpButton><span>{signUp}</span></SignUpButton>
                     }/>
                 </SignedOut>
                 <SignedIn>
                    <UserButton />
      ...
      <main className="relative z-10 w-full border-l border-r border-zinc-800 max-w-[1920px] mx-auto">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 border-b border-zinc-800">
            <div className="col-span-1 lg:col-span-8 p-6 md:p-12 lg:p-24 lg:border-r border-zinc-800 flex flex-col justify-center">
                <span className="text-zinc-500 text-xs uppercase tracking-widest mb-4 block">001 /// Archive Initiation</span>
                ...
                <div className="flex flex-col sm:flex-row gap-6">
                     <Button size="lg" className="bg-[#ccff00] text-black hover:bg-white hover:text-black rounded-none h-14 px-8 font-bold border-2 border-[#ccff00] hover:border-white transition-all uppercase tracking-wide" render={
                        <SignUpButton>
                            <span className="flex items-center gap-2">{hero.cta} <CornerDownRight className="w-4 h-4" /></span>
                        </SignUpButton>
                     }/>
                </div>
            </div>
            <div className="col-span-1 lg:col-span-4 bg-zinc-900/20 relative overflow-hidden hidden lg:flex items-center justify-center">
                <div className="w-full h-full border-t border-zinc-800 lg:border-t-0 p-12 flex flex-col justify-between">
                     <div className="font-anton text-[200px] leading-none text-zinc-900 select-none absolute -top-5 -right-5">A</div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-b border-zinc-800">
             {features.list.map((feature, idx) => (
                 <div key={idx} className="p-8 border-b md:border-b-0 border-r border-zinc-800 hover:bg-zinc-900 transition-colors group h-full">
                     <div className="flex justify-between items-start mb-12">
                         <span className="font-mono text-zinc-600 text-xs">0{idx + 1}</span>
                     ...
                     <h3 className="font-oswald text-2xl uppercase mb-4 text-zinc-100 group-hover:translate-x-1 transition-transform">{feature.title}</h3>
                     <p className="text-xs text-zinc-500 leading-relaxed max-w-[200px]">{feature.desc}</p>
                 </div>
             ))}
        </section>
        ...
        <section className="border-b border-zinc-800">
             <div className="p-6 md:p-12 border-b border-zinc-800 bg-zinc-900/30">
                <span className="text-[#ccff00] font-mono text-xs mb-2 block">/// TARGET_AUDIENCE</span>
                <h2 className="font-anton text-4xl md:text-5xl uppercase text-zinc-200">{useCases.title}</h2>
             </div>
             
             <div className="grid grid-cols-1 lg:grid-cols-3">
                {useCases.list.map((uc, idx) => (
                    <div key={idx} className="p-8 lg:p-12 border-b lg:border-b-0 lg:border-r border-zinc-800 hover:bg-[#ccff00] hover:text-black group transition-colors duration-300">
                        <div className="mb-8">
                             <div className="w-8 h-1 bg-zinc-800 group-hover:bg-black mb-2"></div>
                             <span className="font-mono text-xs text-zinc-500 group-hover:text-black/60">SECTOR 0{idx + 1}</span>
                        </div>
                        <h4 className="font-anton text-3xl uppercase mb-2">{uc.label}</h4>
                        <p className="font-mono text-xs text-zinc-400 group-hover:text-black/80 max-w-xs">{uc.desc}</p>
                    </div>
                ))}
             </div>
        </section>
        ...
        <section className="p-12 md:p-24 text-center border-b border-zinc-800 bg-[#ccff00] text-black">
             ...
             <Button size="lg" className="bg-black text-white hover:bg-white hover:text-black rounded-none h-16 px-12 text-lg font-mono uppercase tracking-widest border-2 border-black transition-all" render={
                <SignUpButton><span>[ INITIATE_NOW ]</span></SignUpButton>
             }/>
        </section>
      </main>
      ...
      <footer className="w-full max-w-[1920px] mx-auto border-l border-r border-b border-zinc-800 bg-zinc-950 p-6 md:p-8">
         <div className="flex flex-col md:flex-row justify-between items-end text-[10px] uppercase font-mono tracking-widest text-zinc-500">
             ...
             <div className="flex gap-8 mt-8 md:mt-0">
                 ...
                 <span className="hover:text-[#ccff00] cursor-pointer">System_Status: Operational</span>
             </div>
         </div>
      </footer>
    </div>
